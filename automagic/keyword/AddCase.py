...
def add_cass(user, password, section_id, case_data):
    ...
    client = testrail.APIClient('http://testrail.wondershare.cn/testail')
    client.user = user
    client.password = password
    result = []
    try:
        for case_id, testrailcaseid, data in case_data:
            # print case_id ,testrailcaseid,data
...
# 添加用例未覆盖
def add_cass2(user, password, section_id, case_data):
    '''
    增加testrail测试用例
    :param user: testrail用户名
    :param password: testrail密码
...
class MyPySql(object):
    ...
    def set_conn(self):
        self.conn = pymysql.connect(host='192.168.11.83', port=3306, user='root', passwd='root', db='autoplat', charset='utf8')
...
if "__main__" == __name__:
    ...
    if user is not None and password is not None and case_ids is not None:
        ...
        # print case_data
        if case_data:
            my_py_sql.execute(u'''SELECT isenabled FROM autoplat_task WHERE id = %s''' % args.task_id)
            case_info = my_py_sql.fetchall()
            # print case_info[0][0]
            if str(case_info[0][0]) == "1":
                add_result = add_cass(user, password, args.section_id, case_data)
            else:
                add_result = add_cass2(user, password, args.section_id, case_data)
