...
def action_ele_2_countnum(action_object, step_desc, value, loc):
    ...
    :return:
    """
    system_time = action_object.find_element(
        ('css', '#setting_systemconsole_container_editNtpSync input.time-picker-input')).get_attribute('placeholder')
    local_time = (
        action_object.find_element(('css', '#setting_systemconsole_container_editNtpSync div.local-time')).text)
...
def action_ipmac_switch(action_object, step_desc, value, loc):
    ...
    :return:
    """
    loc_status = action_object.find_element(
        ('xpath', '//span[text()="IP/MAC地址绑定"]/following-sibling::span//label')).text
    if value == u"开启":
        if loc_status == u"关闭":
...
def action_security_detail(action_object, step_desc, value, loc):
    ...
    if value == u"路由保护模式":
        if securitydetail == u"智能保护模式":
            ...
            Select(action_object.find_element(
                ('css', 'div[ng-show="securitydetail.isEdited"]>select'))).select_by_visible_text(u"路由保护模式")
        ...
        if securitydetail == u"路由保护模式":
            action_object.find_element(('css', 'button[ng-hide="securitydetail.isEdited"]')).click()
            Select(action_object.find_element(
                ('css', 'div[ng-show="securitydetail.isEdited"]>select'))).select_by_visible_text(u"智能保护模式")
            time.sleep(1)
...
def check_description(action_object, step_desc, value, loc):
    ...
    检查描述框的值是否符合预期
    """
    description = action_object.find_element(("xpath", "//textarea[@ng-model='unsavedTask.desc']")).get_attribute(
        "value")
...
def autoScan_ManaualInput(action_object, step_desc, value, loc):
    ...
    action_object.find_element(
        ('xpath', "//form[@name='assetAnalysisForm']//div[contains(.,'资产分析区域')]//button")).click()
...
def controlAutoScan(action_object, step_desc, value, loc):
    ...
    if flag == '1':
        for i in range(600):
            try:
                action_object.find_element(
                    ('xpath', "//div[@class='scan-header']//span[@ng-if='topologyCtrl.showResult']"))
                result = True
            except:
                time.sleep(1)
    elif flag == '2':
...
def fillQuestionnaire(action_object, step_desc, value, loc):
    ...
    for question_block in question_block_list:
        ...
        for question_catagory in question_catagorys:
            question_title_clickables = question_catagory.find_element_by_xpath(
                ".//li[1]")  # 找出所有的title 1. XXX; 2.XXX.....
            # print 'find all div'value
            question_title_clickables = question_title_clickables.find_elements_by_xpath(
                ".//span[@class='checkItem ng-scope']//div")
            if len(question_title_clickables) == 0:
                ...
                question_body_clickables = question_bodys[0].find_elements_by_xpath(
                    ".//span[@class='checkItem']//div[contains(@class,'radioBox')]")
                ...
                try:
                    ...
                    ActionChains(action_object.driver).move_to_element(question_bodys[0].find_element_by_xpath(
                        ".//span[@class='checkItem']//div[not(contains(@class,'ng-hide'))]//input")).perform()
                    question_bodys[0].find_element_by_xpath(
                        ".//span[@class='checkItem']//div[not(contains(@class,'ng-hide'))]//input").send_keys('1')
            else:
...
def setTrafficArea(action_object, step_desc, value, loc):
    ...
    action_object.find_element(('xpath', "//div[@class='project-item-image']")).click()
    action_object.find_element(
        ('xpath', "//button[@class='standard ges-btn-dropdown dropdown-toggle ng-binding']")).click()
...
def checkHelp(action_object, step_desc, value, loc):
    ...
    stored_titles = [[u'如何进行项目管理', u'创建新项目', u'批量操作', u'项目排序', u'项目搜索'],
                     ...
                     [u'如何进行拓扑分析'],
                     [u'如何进行报告管理'],
                     [u'如何进行系统管理', u'恢复出厂设置', u'日志管理', u'系统设置', u'系统升级', u'系统导出', u'版本信息', u'账户管理', u'退出', u'关机',
                      u'重新启动']]
    result = True
    time.sleep(1)
    ...
    for i in range(0, len(the_first_level_nav)):
        ...
        if i != 0:
            ...
            time.sleep(0.2)
        the_selected_first_level_xpath = "//div[contains(@class,'panel-default')][%s]//div[contains(@class,'selected')]" % (
                    i + 1)
        the_opened_second_level_nav = action_object.find_elements(
            ('xpath', "//div[@class='panel-collapse in collapse']//span"))
        first_level_title_left_nav = action_object.find_element(('xpath', the_selected_first_level_xpath)).text
        first_level_title_right = action_object.find_element(
            ('xpath', "//div[@class='help-text ng-scope']/div[contains(@class,'title-name')]")).text
            ...
            if len(the_opened_second_level_nav) > 0:
                for element in range(0, len(the_opened_second_level_nav)):
                    if element + 1 == len(the_opened_second_level_nav):
                        ActionChains(action_object.driver).move_to_element(
                            the_opened_second_level_nav[element]).perform()
                    the_opened_second_level_nav[element].click()
                    time.sleep(0.2)
                    # the_second_title_left_nav = the_opened_second_level_nav[element].text
                    the_second_title_right = action_object.find_element(
                        ('xpath', "//div[@class='help-text ng-scope']//p[contains(@class,'dtitle')]")).text
