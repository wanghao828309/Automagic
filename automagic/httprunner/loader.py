...
def load_debugtalk_module(debugtalk_path):
    ...
    # print sysstr
    if (sysstr == "Windows"):
        path_list=debugtalk_path.split("\\")
    elif (sysstr == "Linux"):
        path_list = debugtalk_path.split("/")
    imported_module = importlib.import_module(".debugtalk","suite."+path_list[-3]+"."+path_list[-2])
...
def load_project_tests(test_path, dot_env_path=None):
    ...
    # load debugtalk.py
    if debugtalk_path:
        project_mapping["debugtalk"] = load_debugtalk_module(debugtalk_path)
    else:
