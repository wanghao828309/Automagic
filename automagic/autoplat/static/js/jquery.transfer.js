    ...
    $.fn.transfer = function(options) {
        ...
        return this.each(function(){
            ...
            for (var i=0; i< transfer.sourceItemList.length; i++) {
                sourceHtml += settings.itemTpl.replace(/\{\{value\}\}/g, transfer.sourceItemList[i].value).replace(/\{\{text\}\}/g, transfer.sourceItemList[i].text).replace(/\{\{apicaseid\}\}/g, transfer.sourceItemList[i].value);
            }
            for (var i=0; i< transfer.targetItemList.length; i++) {
                targetHtml += settings.itemTpl.replace(/\{\{value\}\}/g, transfer.targetItemList[i].value).replace(/\{\{text\}\}/g, transfer.targetItemList[i].text).replace(/\{\{apicaseid\}\}/g, transfer.sourceItemList[i].value);
            }
            ...
            _this.$filterInput.keyup(function(){
                ...
                if (value == '') {
                    ...
                    $itemList=$itemList.find('.transfer-panel-item');
                    //$itemList.find(".transfer-panel-item:not([data-text*='" + value + "'])").hide();//原方法有bug
                    $itemList.each(function(){
                        var tmpVal=$(this).attr("data-text");                   
            ...
            _this.$addBtn.click(function(){
                ...
                $checkedBoxs.each(function(){
                    ...
                    let editLink = ''
                    if(!window.location.href.includes('apisuite')){
                      editLink= '<span class="checkbox-label pull-left">\
                    ...
                    // $parent.remove();//clone 不是穿梭 modfiy by yansl
                });
            ...
            _this.$removeBtn.click(function(){
                ...
                $checkedBoxs.each(function(){
                    var $parent = $(this).parents('.transfer-panel-item');
                    // html += $parent.prop("outerHTML");//clone 仅仅做删除就可以了
                    $parent.remove();
                });
                 ...
                 <div class="transfer-item pull-left ">\
                     <div class="transfer-panel transfer-left-panel">\
                         ...
                         <div class="transfer-panel-body">\
                             ...
                             <div class="checkbox-group transfer-panel-list  is-filterable" id="transfer-left-api">\
                             </div>\
                         </div>\
                 ...
                 <div class="transfer-item pull-left">\
                     <div class="transfer-panel  transfer-right-panel">\
                         ...
                         <div class="transfer-panel-body">\
                             ...
                             <div class="checkbox-group transfer-panel-list  is-filterable" id="transfer-right-api">\
                             </div>\
                         </div>\
                         <p class="transfer-panel-footer">\
                             <label>\
