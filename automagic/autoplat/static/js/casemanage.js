...
$(document).ready(function(){
    ...
    if($('[name="caseproductname"]').val() != ""){
        var productid = $('#selproductid',parent.document).val();
        $.ajax({
            type: "GET",
    ...
    /* 根据产品名称查询关键字列表 */
    $("#selproductid",parent.document).bind("change", function () {
        if($('[name="caseproductname"]').val() != ""){
          var productid = $('#selproductid',parent.document).val();
            $.ajax({
                type: "GET",
                ...
                success: function (result, TextStatus) {
                    ...
                    if (result.length > 0) {
                        for (i = 0; i < result.length; i++) {
                          keyWord.append('<option value="' + result[i].key + '">'+'['+ result[i].productid +']'+ result[i].kwdescr + '</option>');
                        }
                    }
                    // }
                },
