...
def add_product(request):
    if request.method == 'POST':
        ...
        product = Product(name=name, isenabled=isenabled, descr=descr, sortby=sortby, createat=createat,
                          updateat=updateat)
        product.save()
        return HttpResponseRedirect(reverse('productframe'))
        # return
    else:
...
class productFrame(ListView):
    context_object_name = 'productlist'
    template_name = 'productframe.html'
    paginate_by = 10
    productsum = 0
    model = Product
    http_method_names = [u'get', ]
    ...
    def get_context_data(self, **kwargs):
        context = super(productFrame, self).get_context_data(**kwargs)
        namelist = Product.objects.values('name').annotate()
        context['name'] = namelist
        context['productsum'] = self.productsum
        return context
...
class ModuleListIndex(ListView):
    ...
    def get_queryset(self):
        modulelist = Module.objects.all().order_by('-name')
        projectid = self.request.GET.get('projectid')
    ...
    def get_queryset(self):
        ...
        if keyword:
            caselist = caselist.filter(
                Q(pk__icontains=keyword) | Q(casedesc__icontains=keyword) | Q(createat__icontains=keyword))
...
class caseFrame(CaseListIndex):
    context_object_name = 'caselist'
    template_name = 'caseframe.html'
    paginate_by = 10
    # raw_sql = "select * from autoplat_case where projectid_id in (select id from autoplat_project where productid_id in (select productname_id from autoplat_userandproduct where username_id=3));"
    casesum = 0
    model = Case
    ...
    def get_context_data(self, **kwargs):
        context = super(caseFrame, self).get_context_data(**kwargs)
        # namelist = Case.objects.values('casedesc').annotate()
        # context['casedesc'] = namelist
        context['projectlist'] = Project.objects.all().order_by('-sortby')
        context['modulelist'] = Module.objects.all().order_by('-sortby')
...
class ElementListIndex(ListView):
    ...
    def get_queryset(self):
        prodcutid = UserandProduct.objects.filter(username=self.request.user).values('productname')
        elementlist = Element.objects.filter(
            projectid__in=Project.objects.filter(productid__in=prodcutid).values('id')).order_by('-pk')
        prodcutid = self.request.GET.get('check_productname')
        projectid = self.request.GET.get('projectid')
        moduleid = self.request.GET.get('moduleid')
        ...
        if keyword:
            elementlist = elementlist.filter(
                Q(id__icontains=keyword) | Q(location__icontains=keyword) | Q(descr__icontains=keyword))
    ...
    def get_context_data(self, **kwargs):
        ...
        # print context['elementform']
        context['userandproduct'] = UserandProduct.objects.all()
        return context


##add by yansl@wondershare.cn
class elementFrame(ElementListIndex):
    context_object_name = 'elementlist'
    template_name = 'elementframe.html'
    paginate_by = 10
    elementsum = 0
    model = Element
    http_method_names = [u'get']
...
class KeyWordListIndex(ListView):
    ...
    def get_queryset(self):
        ...
        # print prodcutid
        if prodcutid:
            keywordlist = Keyword.objects.filter(
                productid=prodcutid).order_by('-pk')
        else:
            prodcutid = UserandProduct.objects.filter(username=self.request.user).values('productname')
            id = [0]
            for i in prodcutid:
                # print i
                id.append(i["productname"])
            keywordlist = Keyword.objects.filter(
                productid__in=id).order_by('-pk')
...
class keywordFrame(ListView):
    context_object_name = 'keywordlist'
    template_name = 'keywordframe.html'
    paginate_by = 10
    keywordsum = 0
    model = Keyword
    http_method_names = [u'get']
    ...
    def get_context_data(self, **kwargs):
        context = super(keywordFrame, self).get_context_data(**kwargs)
        context['userandproduct'] = UserandProduct.objects.all()
        context['productlist'] = Product.objects.all()
        context['keywordsum'] = self.keywordsum
        return context
...
def add_project(request):
    if request.method == 'POST':
        ...
        project = Project(productid=Product.objects.get(pk=productid), name=name, isenabled=isenabled, version=version,
                          descr=descr, sortby=sortby,
                          createat=createat, updateat=updateat)
        project.save()

        # return HttpResponse('创建成功')
        return HttpResponseRedirect('/setting/product/view/' + str(project_dict.get('productid')))
    else:
        return HttpResponse('创建失败')
...
def update_project(request):
    if request.method == 'POST':
        ...
        p.update(name=name, isenabled=isenabled, descr=descr, version=version, sortby=sortby, updateat=updateat,
...
def add_module(request):
    if request.method == 'POST':
        ...
        return HttpResponseRedirect('/setting/project/view/' + str(projectid))
    else:
        return HttpResponse('创建失败')
...
def update_module(request):
    if request.method == 'POST':
        ...
        return HttpResponseRedirect('/setting/project/view/' + str(post_dict.get('projectid')))
    else:
        return HttpResponse('修改失败')
...
def add_case(request):
    if request.method == 'POST':
        ...
        return HttpResponseRedirect(reverse('caseframe'))
    else:
        ...
        return render(request, 'caseadd.html',
                      {'userandproduct': userandproduct, 'projectlist': projectlist, 'modulelist': modulelist,
                       'caseform': caseform, 'stepform': stepform})
...
def update_case(request, id):
    ...
    if request.method == 'POST':
        ...
        case.update(projectid=projectid, moduleid=moduleid, casedesc=casedesc, isenabled=isenabled,
                    updateat=updateat, dependent=dependent, testrailcaseid=testrailcaseid, updatetime=updatetime,
                    issmoke=issmoke, jiraid=jiraid)
        ...
        for keyword, element, inputtext, descr in zip(keyword_list, element_list, value_list, desc_list):
            ...
            if element != 'None':
                ...
                tcase.step_set.create(stepid=keyword, descr=descr, keywordid=kw, elementid=elementid,
                                      inputtext=inputtext)
            else:
                # print "ddd",element
                tcase.step_set.create(stepid=keyword, descr=descr, keywordid=kw, inputtext=inputtext)
        return HttpResponseRedirect(reverse('keywordframe'))
    else:
        ...
        return render(request, 'caseedit.html',
                      {'project': project, 'modulelist': modulelist, 'elementlist': elementlist,
                       'projectlist': projectlist, 'keywordlist': keywordlist, 'steplist': steplist, 'tcase': tcase})
    ...
    if request.method == 'POST':
        ...
        for keyword, element, inputtext, descr in zip(keyword_list, element_list, value_list, desc_list):
            ...
            if element != 'None':
                # print "ccc",element
                elementid = Element.objects.get(pk=int(element))
                case.step_set.create(stepid=keyword, descr=descr, keywordid=kw, elementid=elementid,
                                     inputtext=inputtext)
            else:
                # print "ddd",element
                case.step_set.create(stepid=keyword, descr=descr, keywordid=kw, inputtext=inputtext)
        ...
        return render(request, 'casecopy.html',
                      {'project': project, 'modulelist': modulelist, 'elementlist': elementlist,
                       'projectlist': projectlist, 'keywordlist': keywordlist, 'steplist': steplist, 'tcase': tcase})
...
def add_element(request):
    if request.method == 'POST':
        ...
        projectid = request.POST['ele_add_projectid']
        moduleid = request.POST['selmoduleid']
        ...
        ele = Element(moduleid=mid, projectid=pid, descr=descr, locmode=locmode, location=location, createat=createat,
                      updateat=updateat)
        ele.save()
        # return HttpResponse(descr + '@' + locmode + '@' + location + '@' + str(mid.pk))
        # return HttpResponse('添加元素成功。')
        return HttpResponseRedirect(reverse('elementframe'))
    else:
...
def update_element(request):
    if request.method == 'POST':
        ...
        e.update(descr=descr, projectid=projectid, moduleid=moduleid, locmode=locmode, location=location,
                 updateat=updateat, updatetime=updatetime)
        return HttpResponseRedirect(reverse('elementframe'))
    else:
        return HttpResponse('修改失败')
...
def get_product(request):
    ...
    pdlist = dbUtil.get_productObj_byUser(request.user.username)
    productlist = []
    # print pdlist
    if pdlist:
        for pObj in pdlist:
            product = {}
            product['key'] = pObj.productname.id
            product['value'] = pObj.productname.name
...
def get_element(request):
    elementlist = []
    projectid = request.GET['projectid']
    elelista = Element.objects.raw(
        "select id,(select name from autoplat_module where id= moduleid_id) as modulename,locmode,location,descr from autoplat_element where projectid_id=" + projectid)
    for i in elelista:
        element = {}
...
# def copy_res():
#     pwd = os.getcwd()
#     path = pwd + "\\keyword\\result"
#     cmd = r"Xcopy " + path + r" E:\work\apache-tomcat-8.5.34\webapps\result  /s /e /y"
#     os.system(cmd)
...
def get_task_history(request):
    ...
    if task.tasktype == '1':
        ...
        for i in taskhistory:
            ...
            i.reporturl = insert(i.reporturl)
        ...
        try:
            ...
            # print task.build_name
            for i in jenkinsjob['builds'][:20]:
                ...
                th['result'] = bi['result']
                th['user'] = "由用户 {} 启动".format(user_id)
                try:
                    th['user'] = bi['actions'][0]['causes'][0]['shortDescription']
                except Exception:
                    pass
                th['consoleurl'] = bi['url'] + 'console'
                th['reporturl'] = 'http://192.168.11.83:8080/job/' + build_name + r"/ws/keyword/result/" + \
                                  th['starttime'].split(" ")[0]
                # if len(bi['artifacts'])>0:
                #     th['reporturl'] = url+testreport(bi['artifacts'][0]['relativePath'])
                # else:
                #     th['reporturl'] = i["url"].split(build_name)[0]+"test" + build_name + r"/ws/keyword/result/" + insert(i.reporturl)
                task_history.append(th)
            return HttpResponse(json.dumps(task_history))
        except:
            return HttpResponse(json.dumps(task_history))
...
def run_case(request):
    ...
    if depentid == u'':
        os.system("python keyword/TestSuite.py -c %d" % int(caseid))
    else:
        os.system("python keyword/TestSuite.py -c " + depentid + "," + caseid)
    # print caseid
    ...
    def get_queryset(self):
        ...
        if keyword:
            tasklist = tasklist.filter(
                Q(pk__icontains=keyword) | Q(taskname__icontains=keyword) | Q(createat__icontains=keyword))
...
class taskFrame(TaskListIndex):
    context_object_name = 'tasklist'
    template_name = 'taskiframe.html'
    paginate_by = 10
    # raw_sql = "select * from autoplat_case where projectid_id in (select id from autoplat_project where productid_id in (select productname_id from autoplat_userandproduct where username_id=3));"
    tasksum = 0
    model = Task
    ...
    def get_context_data(self, **kwargs):
        context = super(taskFrame, self).get_context_data(**kwargs)
        # tasklist = Task.objects.values('taskname').annotate()
        context['tasksum'] = self.tasksum
        # context['productlist'] = Product.objects.all().order_by('-sortby')
        context['userandproduct'] = UserandProduct.objects.all()
...
def add_task(request):
    if request.method == 'POST':
        ...
        jen_task_id = post_dict['Jtask_id']
        if int(issmoke):
            issmoke = True
        else:
            issmoke = False
        ...
        createat = request.user.username
        updateat = request.user.username
        task = Task(taskname=taskname, testrailrunid=runid, testrailsuites=suitesid, testsectionid=sectionid,
                    projectid=projectid,
                    ...
                    jen_task_id=jen_task_id, isenabled=isenabled)
        task.save()
        # if post_dict.has_key('codename'):
        #     codename_list, codedescr_list, codevalue_list = post_dict.getlist("codename"), post_dict.getlist(
        #         "codedescr"), post_dict.getlist("codevalue")
        ...
        return HttpResponseRedirect(reverse('taskiframe'))
    else:
        userandproduct = UserandProduct.objects.all()
        projectlist = Project.objects.all().order_by('-sortby')
        modulelist = Module.objects.all().order_by('-sortby')
...
def update_task(request, id):
    ...
    if request.method == 'POST':
        ...
        task.update(projectid=projectid, taskname=taskname, tasktype=tasktype, status=0, caselist=caseids,
                    testrailrunid=testrailrunid,
                    api_token=api_token, jenkins_server_url=jenkins_server_url, updateat=updateat,
                    updatetime=updatetime,
                    user_id=user_id, build_name=build_name, testsectionid=testsectionid, testrailsuites=suitesid,
                    issmoke=issmoke, jen_task_id=jen_task_id, isenabled=isenabled)
        ...
        return render(request, 'taskedit.html',
                      {'taskinfo': taskinfo, 'codelist': codelist, 'casesum': casesum, 'userandproduct': userandproduct,
                       'projectlist': projectlist, 'modulelist': modulelist, 'caselist': caselist})
...
def run_task(request):
    ...
    if tasktype == '1':
        if runid:
            os.system("python keyword/TestSuite.py -t %s -u %s -r %s &" % (taskid, userid, runid))
        else:
            os.system("python keyword/TestSuite.py -t %s -u %s &" % (taskid, userid))
        # copy_res()
    elif tasktype == '2':
        os.system("python keyword/AddCase.py -t %s -u %s -s %s &" % (taskid, userid, sectionid))
        starttime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
        # print starttime
        # task.status = 1
        # task.save()
        caselist = task.caselist
        th = Taskhistory(taskid=task, userid=request.user, tasktype='2', taskname=task.taskname, starttime=starttime,
                         reporturl=caselist)
        th.save()
    elif tasktype == '3':
        jenkins_server_url = task.jenkins_server_url
        ...
        api_token = task.api_token
        build_name = task.build_name
        jen_task_id = task.jen_task_id
        RunId = task.testrailrunid
        server = jenkins.Jenkins(jenkins_server_url, user_id, api_token)
        starttime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
        if jen_task_id is None:
            server.build_job(build_name)
        else:
            if RunId == "" or RunId is None:
                RunId = 0
            p = {"TaskId": str(jen_task_id), "TrunId": str(RunId)}
            # print(p)
            server.build_job(build_name, parameters=p)
        # 改变执行状态
        task.status = 1
        jenkinsjob = server.get_job_info(build_name)
        num = jenkinsjob['lastBuild']['number']
        # print num
        time.sleep(15)
        while True:
            building = server.get_build_info(build_name, num + 1)['building']
            # print building
            if not building:
                task.status = 2
                break
    ...
    def get_queryset(self):
        ...
        if keyword:
            userlist = userlist.filter(
                Q(username__icontains=keyword) | Q(email__icontains=keyword) | Q(mobile__icontains=keyword))
...
class userFrame(UserListIndex):
    context_object_name = 'userlist'
    template_name = 'userframe.html'
    paginate_by = 10
    model = User
    usersum = 0
    http_method_names = [u'get']
    ...
    def get_context_data(self, **kwargs):
        context = super(userFrame, self).get_context_data(**kwargs)
        # userlist = User.objects.values('username').annotate()
        context['productlist'] = Product.objects.all().order_by('-sortby')
        context['userandproduct'] = UserandProduct.objects.all()
        context['usersum'] = self.usersum
...
def add_user(request):
    if request.method == 'POST':
        ...
        user = User(username=username, password=password, realname=realname, email=email, mobile=mobile, dept=dept,
                    is_active=is_active, is_admin=is_admin, testrailuser=testrailuser, testrailpass=testrailpass)
        user.save()
        return HttpResponseRedirect(reverse('userframe'))
    else:
        return HttpResponse('创建失败')
...
def update_user(request):
    if request.method == 'POST':
        ...
        if user_dict.get('password') == '':
            ...
            password = make_password(user_dict.get('password'), None, 'pbkdf2_sha256')
            u.update(username=username, password=password, realname=realname, dept=dept, email=email, mobile=mobile,
                     is_active=is_active,
                     is_admin=is_admin, testrailuser=testrailuser, testrailpass=testrailpass)
            return HttpResponseRedirect(reverse('userframe'))
    else:
...
def product_user(request):
    if request.method == 'POST':
        ...
        for i in range(0, len(username)):
            userandproduct = UserandProduct(username=User.objects.get(pk=username[i]),
                                            productname=Product.objects.get(pk=productname))
            userandproduct.save()
        return HttpResponseRedirect(reverse('userframe'))
    else:
...
def ApiListIndex(request):
    return render_to_response('apilist.html')


def apiFrame(request):
    return render_to_response('apiframe.html')


def apiInfo(request):
    return render_to_response('apiInfo.html')


def apiAdd(request):
    return render_to_response('apiAdd.html')


def apiEdit(request):
    return render_to_response('apiEdit.html')


def apiCopy(request):
    return render_to_response('apiCopy.html')
...
def apirun(request):
    return render_to_response('apiRun.html')


def apicaseAdd(request):
    return render_to_response('apicaseadd.html')


def apicaseEdit(request):
    return render_to_response('apicaseedit.html')


def apicaseDel(request):
    return render_to_response('apicaseframe.html')


def apicaserun(request):
    return render_to_response('apiCaseRun.html')


def apisuiteAdd(request):
    return render_to_response('apisuiteadd.html')


def apisuiteEdit(request):
    return render_to_response('apisuiteedit.html')


def apisuiteDel(request):
    return render_to_response('apisuiteframe.html')


def apisuiterun(request):
    return render_to_response('apisuiterun.html')


def apisuitelist(request):
    return render_to_response('apisuitelist.html')


def apicaselist(request):
    return render_to_response('apicaselist.html')


def apicaseframe(request):
    return render_to_response('apicaseframe.html')


def apisuiteframe(request):
    return render_to_response('apisuiteframe.html')


def hostlist(request):
    return render_to_response('Hostlist.html')


def hostframe(request):
    return render_to_response('Hostframe.html')
...
class hostFrame(HostListIndex):
    context_object_name = 'hostframe'
    template_name = 'hostframe.html'
    paginate_by = 10
    # raw_sql = "select * from autoplat_case where projectid_id in (select id from autoplat_project where productid_id in (select productname_id from autoplat_userandproduct where username_id=3));"
    casesum = 0
    model = Case
...
class ApiTaskListIndex(ListView):
    context_object_name = 'apitasklist'
    template_name = 'apitasklist.html'
    paginate_by = 10
    # raw_sql = "select * from autoplat_case where projectid_id in (select id from autoplat_project where productid_id in (select productname_id from autoplat_userandproduct where username_id=3));"
    casesum = 0
    model = Case
...
class apitaskFrame(CaseListIndex):
    context_object_name = 'apitaskframe'
    template_name = 'apitaskframe.html'
    paginate_by = 10
    # raw_sql = "select * from autoplat_case where projectid_id in (select id from autoplat_project where productid_id in (select productname_id from autoplat_userandproduct where username_id=3));"
    casesum = 0
    model = Case
